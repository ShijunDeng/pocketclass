...
<script language="javascript"
	...
	function confirmOp(tag, msg, obj,optiontype) {
		...
		//根据不同的请求类型动态的改变action
			document.form1.action="${pageContext.request.contextPath}/servlet/MsgServlet?option="+optiontype+"&idLoca=003";
			obj.form.submit();
			return true;
		}
	}
								...
								<c:when test="${msgs!=null&&requestScope.pageCount>0}">
											...
											<c:forEach items="${msgs}" var="msg">
												<tr class="align_center">
													<td><input type="checkbox" name="uuids"
														...
														value="${msg.sendUserName}&${msg.sendTime}" />
													</td>
													<td><a
													...
													<td><a href="#">${msg.sendUserName}</a></td>
													<td><a href="#">${msg.addreUserName}</a></td>
													<c:set var="sendTime" value="${msg.sendTime}"></c:set>
													<td>${fn:replace(sendTime," ","<br>")}</td>
												</tr>
											</c:forEach>
										</table>	<br>
										<input type="button" value="转入垃圾箱" class="btn-img btn-s1"
									...
									<span class="op_form"> <input type="checkbox"
										...
										<c:choose>
											...
											<c:otherwise>
												<a
													href="${pageContext.request.contextPath}/servlet/MsgServlet?option=first&idLoca=003">首页</a>
											</c:otherwise>
										</c:choose> <c:choose>
											<c:when test="${requestScope.currentPage > 1}">
												<a
													href="${pageContext.request.contextPath}/servlet/MsgServlet?option=prior&idLoca=003">上一页</a>
											</c:when>
											<c:otherwise>
											...
											<c:choose>
												...
												<c:otherwise>
													<a
														href="${pageContext.request.contextPath}/servlet/MsgServlet?option=jump&idLoca=003&page=${i}">${i}</a>
												</c:otherwise>
											</c:choose>
										</c:forEach> <c:choose>
											<c:when
												...
												<a
													href="${pageContext.request.contextPath}/servlet/MsgServlet?option=next&idLoca=003">下一页</a>
											</c:when>
											<c:otherwise>
											...
											<c:otherwise>
												<a
													href="${pageContext.request.contextPath}/servlet/MsgServlet?option=end&idLoca=003">尾页</a>
											</c:otherwise>
										</c:choose> </span>
									<a href='javascript:void(0)'
										onclick="toConfirm('该操作将删除所有信息且无法恢复，确认清空?', '${pageContext.request.contextPath}/servlet/MsgServlet?option=flush&idLoca=003')">清空</a>
