								...
								<c:when test="${msgs!=null&&requestScope.pageCount>0}">
										...
										<table width="100%" cellspacing="0" cellpadding="0" border="1">
											...
											<c:forEach items="${msgs}" var="msg">
												<tr class="align_center">
													...
													<td><a
														href="${pageContext.request.contextPath}/servlet/MsgContentServlet?option=content&msg=${msg.sendUserName}^${msg.sendTime}&idLoca=001">${msg.title}<c:if
																test="${msg.isRead eq 'NO'}">
																<span style="color:red"><c:out value="[未读]"></c:out>
																</span>
															</c:if> </a>
													</td>
									...
									<span class="op_form"> <input type="checkbox"
										...
										<c:choose>
											...
											<c:otherwise>
												<a
													href="${pageContext.request.contextPath}/servlet/MsgServlet?option=first&idLoca=001">首页</a>
											</c:otherwise>
										</c:choose> <c:choose>
											<c:when test="${requestScope.currentPage > 1}">
												<a
													href="${pageContext.request.contextPath}/servlet/MsgServlet?option=prior&idLoca=001">上一页</a>
											</c:when>
											<c:otherwise>
											...
											<c:choose>
												...
												<c:otherwise>
													<a
														href="${pageContext.request.contextPath}/servlet/MsgServlet?option=jump&idLoca=001&page=${i}">${i}</a>
												</c:otherwise>
											</c:choose>
										</c:forEach> <c:choose>
											<c:when
												...
												<a
													href="${pageContext.request.contextPath}/servlet/MsgServlet?option=next&idLoca=001">下一页</a>
											</c:when>
											<c:otherwise>
											...
											<c:otherwise>
												<a
													href="${pageContext.request.contextPath}/servlet/MsgServlet?option=end&idLoca=001">尾页</a>
											</c:otherwise>
										</c:choose> </span>
									<a href='javascript:void(0)'
										onclick="toConfirm('该操作将删除所有信息且无法恢复，确认清空?', '${pageContext.request.contextPath}/servlet/MsgServlet?option=flush&idLoca=001')">清空</a>
