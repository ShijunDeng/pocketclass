							...
							<c:choose>
								<c:when test="${msgs!=null&&requestScope.pageCount>0}">
									<form method="post" name="form1">
										...
										<br> <input type="button" value="转入收藏箱"
									...
									<span class="op_form"> <input type="checkbox"
										...
										<c:choose>
											...
											<c:otherwise>
												<a
													href="${pageContext.request.contextPath}/servlet/MsgServlet?option=first&idLoca=004">首页</a>
											</c:otherwise>
										</c:choose> <c:choose>
											<c:when test="${requestScope.currentPage > 1}">
												<a
													href="${pageContext.request.contextPath}/servlet/MsgServlet?option=prior&idLoca=004">上一页</a>
											</c:when>
											<c:otherwise>
											...
											<c:choose>
												...
												<c:otherwise>
													<a
														href="${pageContext.request.contextPath}/servlet/MsgServlet?option=jump&idLoca=004&page=${i}">${i}</a>
												</c:otherwise>
											</c:choose>
										</c:forEach> <c:choose>
											<c:when
												...
												<a
													href="${pageContext.request.contextPath}/servlet/MsgServlet?option=next&idLoca=004">下一页</a>
											</c:when>
											<c:otherwise>
											...
											<c:otherwise>
												<a
													href="${pageContext.request.contextPath}/servlet/MsgServlet?option=end&idLoca=004">尾页</a>
											</c:otherwise>
										</c:choose> </span>
									<a href='javascript:void(0)'
										onclick="toConfirm('该操作将删除所有信息且无法恢复，确认清空?', '${pageContext.request.contextPath}/servlet/MsgServlet?option=flush&idLoca=004')">清空</a>
