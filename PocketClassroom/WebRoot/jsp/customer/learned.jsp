							...
							<c:choose>
								<c:when
									...
									<table width="100%" cellspacing="0" cellpadding="0" border="1">
										<tbody>
											<tr>
												<th width="15%">课程系列名称</th>
												<th width="20%">所属分类</th>
												<th width="7%">总课时</th>
												<th width="10%">讲师</th>
												<th width="18%">开始学习时间</th>
												<th width="18%">完成学习时间</th>
												<th width="12%">操作</th>
											</tr>
										</tbody>
										...
										<c:forEach items="${learnProgresses}" var="learnProgress">
											<tr class="align_center">
												...
												<c:set var="endTime" value="${learnProgress.endTime}"></c:set>
												<td>${fn:replace(endTime," ","<br>") }</td>
												<td style="font-size:14px;font-family:幼圆"><a
													href='javascript:void(0)'
													onclick="toConfirm('确认继续学习(本系列视频的学习记录将被新纪录覆盖)?', '${pageContext.request.contextPath}/servlet/LearnServlet?idVideoSeries=${learnProgress.idVideoSeries}&isEnd=YES&option=changeToLearn')">继续学习</a><br>
													<br /> <a href="#">播放记录</a><br />
												</td>
											</tr>
										</c:forEach>
									...
									<span class="op_form"> <c:choose>
											...
											<c:otherwise>
												<a
													href="${pageContext.request.contextPath}/servlet/LearnServlet?isEnd=NO?option=first&isEnd=YES">首页</a>
											</c:otherwise>
										</c:choose> <c:choose>
											<c:when test="${requestScope.currentPage > 1}">
												<a
													href="${pageContext.request.contextPath}/servlet/LearnServlet?option=prior&isEnd=YES">上一页</a>
											</c:when>
											<c:otherwise>
											...
											<c:choose>
												...
												<c:otherwise>
													<a
														href="${pageContext.request.contextPath}/servlet/LearnServlet?option=jump&page=${i}&isEnd=YES">${i}</a>
												</c:otherwise>
											</c:choose>
										</c:forEach> <c:choose>
											<c:when
												...
												<a
													href="${pageContext.request.contextPath}/servlet/LearnServlet?option=next&isEnd=YES">下一页</a>
											</c:when>
											<c:otherwise>
											...
											<c:otherwise>
												<a
													href="${pageContext.request.contextPath}/servlet/LearnServlet?option=end&isEnd=YES">尾页</a>
											</c:otherwise>
										</c:choose> </span>
									<a href='javascript:void(0)'
										onclick="toConfirm('该操作无法恢复，确认清空?', '${pageContext.request.contextPath}/servlet/LearnServlet?option=flush&isEnd=YES')">清空</a>
								...
								<div class="clear"></div>
									<table width="100%" cellspacing="0" cellpadding="0" border="0">
										<tbody>
											<tr>
												<th width="15%"></th>
												<th width="70%"><br> <br>暂无正在学习课程，赶紧选择自己喜欢的课程开始学习吧!<br>
													<br> <br> <br> <br></th>
												<th width="15%"></th>
											</tr>
										</tbody>
